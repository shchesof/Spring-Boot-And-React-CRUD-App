...
class PublishingHouseEdit extends Component {
    ...
    async handleSubmit(event) {
        if (this.validateState()) {
            ...
            await fetch('/publishingHouses' + (item.id ? '/' + item.id : ''), {
    ...
    render() {
        const {item} = this.state;
        const title = <h2>{item.id ? 'Edit Publishing House' : 'Add Publishing House'}</h2>;
...
export default withRouter(PublishingHouseEdit);